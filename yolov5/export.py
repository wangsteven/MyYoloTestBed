...
def export_torchscript(model, img, file, optimize):
    ...
    try:
        ...
        f = (file.with_suffix(".torchscript.ptl") if optimize else file.with_suffix(".torchscript.pt"))
        MODEL_PATH=''
        model_pth = os.path.join(MODEL_PATH, f) 
        ts = torch.jit.trace(model, img, strict=False)
        #(optimize_for_mobile(ts) if optimize else ts).(_save_for_lite_interpreter(f) if optimize else save(f))
        
        traced_script_module_optimized = (optimize_for_mobile(ts) if optimize else ts)
        exported_optimized_scripted_module = traced_script_module_optimized._save_for_lite_interpreter(model_pth) if optimize else save(model_pth) 
